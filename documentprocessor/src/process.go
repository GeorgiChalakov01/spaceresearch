...
func CallOllama(model, prompt string, imageBase64 string) (string, error) {
    ...
    for {
        ...
        if err := decoder.Decode(&chunk); err != nil {
            ...
            return "", fmt.Errorf("failed to decode response chunk: %w", err)
        }
...
func ProcessDocument(details []byte) {
    ...
    for _, page := range pageContents {
        ...
        explanationOfVisuals, err := CallOllama(model, prompt, imageInBase64)
        if err != nil {
